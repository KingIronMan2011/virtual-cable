(function() {
	try {
		var a = require("electron").app;
		if (a && a.isPackaged) module.paths.unshift(require("path").join(process.resourcesPath, "node_modules"));
	} catch (_) {}
})();
...
*/
var engine = require("virtual-cable-engine");
/** Emits `("level", tunnelId: string, level: number)` at ~20 fps per active tunnel. */
var levelEmitter = new node_events.EventEmitter();
engine.registerLevelCallback((tunnelId, level) => {
...
function createTunnel(tunnelId, inputs, outputDeviceId, framesPerBuffer = 0, requestedChannels = null, ducking = {
	enabled: false,
	amount: .15,
	release: 1e3
}) {
	engine.createTunnel(tunnelId, inputs, outputDeviceId, framesPerBuffer, requestedChannels ?? 0, ducking);
}
function destroyTunnel(tunnelId) {
	engine.destroyTunnel(tunnelId);
}
function destroyAllTunnels() {
	engine.destroyAllTunnels();
}
function reloadAllTunnels(framesPerBuffer) {
	engine.reloadAllTunnels(framesPerBuffer);
}
function setTunnelMuted(tunnelId, muted) {
	engine.setTunnelMuted(tunnelId, muted);
}
function setTunnelGain(tunnelId, gain) {
	engine.setTunnelGain(tunnelId, gain);
}
function setTunnelInputGain(tunnelId, inputIndex, gain) {
	engine.setTunnelInputGain(tunnelId, inputIndex, gain);
}
function setTunnelInputPriority(tunnelId, inputIndex, priority) {
	engine.setTunnelInputPriority(tunnelId, inputIndex, priority);
}
function setTunnelDucking(tunnelId, ducking) {
	engine.setTunnelDucking(tunnelId, ducking);
}
function getTunnelSampleRate(tunnelId) {
	return engine.getTunnelSampleRate(tunnelId);
}
function getTunnelChannelCount(tunnelId) {
...
//#endregion
//#region src/vbaudio/installer.ts
...
var VB_DOWNLOAD_URL = "https://download.vb-audio.com/Download_CABLE/VBCABLE_Driver_Pack45.zip";
async function isVBAudioInstalled() {
	try {
		if (getAudioDevices().some((d) => d.name.toLowerCase().includes("cable"))) return true;
	} catch {}
	for (const key of VB_REGISTRY_KEYS) try {
...
async function openInstallPage() {
	await electron.shell.openExternal("https://www.vb-audio.com/Cable/");
}
/**
...
async function downloadAndInstall(onProgress) {
	const destDir = (0, node_path.join)((0, node_os.tmpdir)(), `vbcable-${Date.now()}`);
	await (0, node_fs_promises.mkdir)(destDir, { recursive: true });
	const zipPath = (0, node_path.join)(destDir, "VBCABLE_Driver_Pack45.zip");
	await new Promise((resolve, reject) => {
		const get = (url) => {
			(url.startsWith("https") ? node_https.default : node_http.default).get(url, (res) => {
				if ((res.statusCode === 301 || res.statusCode === 302) && res.headers.location) return get(res.headers.location);
				if (res.statusCode !== 200) return reject(/* @__PURE__ */ new Error(`Download failed: HTTP ${res.statusCode}`));
				const total = parseInt(res.headers["content-length"] ?? "0", 10);
				let received = 0;
				const file = (0, node_fs.createWriteStream)(zipPath);
				res.on("data", (chunk) => {
					received += chunk.length;
					if (total > 0) onProgress("downloading", Math.round(received / total * 100));
				});
				res.pipe(file);
	...
	new import_adm_zip.default(zipPath).extractAllTo(destDir, true);
	onProgress("launching");
	const exeName = process.arch === "x64" ? "VBCABLE_Setup_x64.exe" : "VBCABLE_Setup.exe";
	const exePath = (0, node_path.join)(destDir, exeName);
	if (!(0, node_fs.existsSync)(exePath)) throw new Error(`Could not find ${exeName} in downloaded package.`);
	await electron.shell.openPath(exePath);
}
//#endregion
//#region src/store/tunnelStore.ts
function storePath$1() {
	const dir = electron.app.getPath("userData");
	(0, node_fs.mkdirSync)(dir, { recursive: true });
	return (0, node_path.join)(dir, "tunnels.json");
}
/** Migrate a raw persisted object to the current Tunnel shape. */
function migrate(raw) {
	let inputs;
	if (Array.isArray(raw.inputs) && raw.inputs.length > 0) inputs = raw.inputs.map((inp) => ({
	...
	return {
		...
		duckingAmount: typeof raw.duckingAmount === "number" ? raw.duckingAmount : .15,
		duckingRelease: typeof raw.duckingRelease === "number" ? raw.duckingRelease : 1e3
	};
}
function loadTunnels() {
	try {
		const raw = (0, node_fs.readFileSync)(storePath$1(), "utf8");
...
function saveTunnels(tunnels) {
	const toSave = tunnels.map((t) => ({
		...t,
		active: false,
		muted: false
	}));
	(0, node_fs.writeFileSync)(storePath$1(), JSON.stringify(toSave, null, 2), "utf8");
}
//#endregion
...
function storePath() {
	const dir = electron.app.getPath("userData");
	(0, node_fs.mkdirSync)(dir, { recursive: true });
	return (0, node_path.join)(dir, "settings.json");
}
function loadSettings() {
	try {
		const raw = (0, node_fs.readFileSync)(storePath(), "utf8");
		return {
			...DEFAULTS,
			...JSON.parse(raw)
		};
...
function saveSettings(settings) {
	(0, node_fs.writeFileSync)(storePath(), JSON.stringify(settings, null, 2), "utf8");
}
//#endregion
//#region src/updater/updater.ts
...
function setupUpdater(mainWin) {
	...
	const feedUrl = `https://vc-upd.kingironman.dev/update/${process.platform}/${electron.app.getVersion()}`;
	try {
	...
	electron.autoUpdater.on("update-not-available", () => {
		...
		setTimeout(() => push({ status: "idle" }), 4e3);
	});
	electron.autoUpdater.on("update-downloaded", (_event, _releaseNotes, releaseName) => {
		push({
			status: "ready",
			version: releaseName
		});
...
function checkForUpdates() {
	if (!electron.app.isPackaged) {
		push({ status: "not-available" });
		setTimeout(() => push({ status: "idle" }), 3e3);
		return;
	}
...
var ANALYTICS_ENDPOINT = "https://virtual-cable-analytics.kingironman2011.workers.dev/event";
var cachedHwid = null;
function getHwid() {
	if (cachedHwid) return cachedHwid;
	const mac = Object.values(node_os.default.networkInterfaces()).flat().find((iface) => iface && !iface.internal && iface.mac !== "00:00:00:00:00:00")?.mac ?? "";
	const raw = [
		node_os.default.hostname(),
		node_os.default.platform(),
		node_os.default.arch(),
		node_os.default.cpus()[0]?.model ?? "",
		mac
	].join("|");
	cachedHwid = (0, node_crypto.createHash)("sha256").update(raw).digest("hex");
	return cachedHwid;
}
...
function assetPath(file) {
	return electron.app.isPackaged ? node_path.default.join(process.resourcesPath, "assets", file) : node_path.default.join(__dirname, "../../assets", file);
}
function createTray(win) {
	tray = new electron.Tray(process.platform === "darwin" ? assetPath("icon.png") : assetPath("icon.ico"));
...
function createWindow() {
	...
	win.on("close", (event) => {
		if (loadSettings().minimizeToTray && !isQuitting) {
	...
	createTray(win);
	levelEmitter.on("level", (tunnelId, level) => {
		if (!win.isDestroyed()) win.webContents.send("audio:level", tunnelId, level);
	});
	setupUpdater(win);
	win.webContents.once("did-finish-load", () => {
		if (loadSettings().autoUpdate) checkForUpdates();
	});
}
electron.ipcMain.handle("audio:getDevices", () => getAudioDevices());
electron.ipcMain.handle("audio:getAudioApps", () => listAudioApps());
var effectiveFpb = (s) => s.experimentalFeatures && s.expLatency ? s.bufferSize : 0;
electron.ipcMain.handle("audio:createTunnel", (_event, id, inputs, outputId, channelCount, ducking) => {
	createTunnel(id, inputs, outputId, effectiveFpb(loadSettings()), channelCount, ducking);
});
electron.ipcMain.handle("audio:getTunnelSampleRate", (_event, id) => getTunnelSampleRate(id));
electron.ipcMain.handle("audio:getTunnelChannelCount", (_event, id) => getTunnelChannelCount(id));
electron.ipcMain.handle("audio:setTunnelMuted", (_event, id, muted) => {
	setTunnelMuted(id, muted);
});
electron.ipcMain.handle("audio:setTunnelGain", (_event, id, gain) => {
	setTunnelGain(id, gain);
});
electron.ipcMain.handle("audio:setTunnelInputGain", (_event, id, inputIndex, gain) => {
	setTunnelInputGain(id, inputIndex, gain);
});
electron.ipcMain.handle("audio:setTunnelInputPriority", (_event, id, inputIndex, priority) => {
	setTunnelInputPriority(id, inputIndex, priority);
});
electron.ipcMain.handle("audio:setTunnelDucking", (_event, id, ducking) => {
	setTunnelDucking(id, ducking);
});
electron.ipcMain.handle("audio:destroyTunnel", (_event, id) => {
	destroyTunnel(id);
});
electron.ipcMain.handle("vbaudio:checkInstalled", () => isVBAudioInstalled());
electron.ipcMain.handle("vbaudio:install", () => openInstallPage());
electron.ipcMain.handle("vbaudio:downloadAndInstall", async (event) => {
	await downloadAndInstall((stage, pct) => {
		event.sender.send("vbaudio:progress", {
			stage,
			pct
		});
	});
});
electron.ipcMain.handle("store:loadTunnels", () => loadTunnels());
electron.ipcMain.handle("store:exportLayout", async (_event, json) => {
	const result = await electron.dialog.showSaveDialog({
		title: "Export Cable Layout",
		defaultPath: "virtual-cable-layout.json",
		filters: [{
			name: "Virtual Cable Layout",
			extensions: ["json"]
		}]
	});
	if (!result.canceled && result.filePath) {
		(0, node_fs.writeFileSync)(result.filePath, json, "utf8");
		return true;
	}
	return false;
});
electron.ipcMain.handle("store:importLayout", async () => {
	...
	if (!result.canceled && result.filePaths[0]) return (0, node_fs.readFileSync)(result.filePaths[0], "utf8");
	return null;
});
electron.ipcMain.handle("store:saveTunnels", (_event, tunnels) => saveTunnels(tunnels));
electron.ipcMain.handle("settings:load", () => loadSettings());
electron.ipcMain.handle("settings:save", (_event, settings) => {
	const prev = loadSettings();
	saveSettings(settings);
	if (effectiveFpb(settings) !== effectiveFpb(prev)) reloadAllTunnels(effectiveFpb(settings));
});
electron.ipcMain.handle("update:check", () => checkForUpdates());
electron.ipcMain.handle("update:install", () => installUpdate());
electron.ipcMain.handle("update:getState", () => getUpdateState());
electron.app.on("ready", async () => {
	if (process.platform !== "win32") {
		const flagFile = node_path.default.join(electron.app.getPath("userData"), "registered.flag");
		if (!(0, node_fs.existsSync)(flagFile)) {
			(0, node_fs.writeFileSync)(flagFile, "");
