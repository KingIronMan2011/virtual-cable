...
static napi_threadsafe_function g_levelTsfn = nullptr;

static void LevelTsfnCallback(napi_env env, napi_value jsCb,
                                void* /*ctx*/, void* data) {
    auto* ld = static_cast<LevelData*>(data);
    if (!ld) return;
    if (!env) { delete ld; return; }

    napi_value tunnelIdVal, levelVal;
    napi_create_string_utf8(env, ld->tunnelId.c_str(), ld->tunnelId.size(),
                            &tunnelIdVal);
    napi_create_double(env, (double)ld->level, &levelVal);

    napi_value global, result;
    napi_get_global(env, &global);
    napi_value args[] = {tunnelIdVal, levelVal};
    napi_call_function(env, global, jsCb, 2, args, &result);
    delete ld;
}

/* ── N-API helper macros ───────────────────────────────────────────────── */

#define NAPI_CALL(call) do { napi_status s = (call); if (s != napi_ok) return nullptr; } while(0)

static napi_value NapiGetAudioDevices(napi_env env, napi_callback_info) {
    auto devices = Engine::getAudioDevices();
    napi_value arr;
    napi_create_array_with_length(env, devices.size(), &arr);
    for (size_t i = 0; i < devices.size(); i++) {
        napi_value obj, id, name, maxIn, maxOut;
        napi_create_object(env, &obj);
        napi_create_int32(env, devices[i].id, &id);
        napi_create_string_utf8(env, devices[i].name.c_str(),
                                devices[i].name.size(), &name);
        napi_create_int32(env, devices[i].maxInputChannels, &maxIn);
        napi_create_int32(env, devices[i].maxOutputChannels, &maxOut);
        napi_set_named_property(env, obj, "id", id);
        napi_set_named_property(env, obj, "name", name);
        napi_set_named_property(env, obj, "maxInputChannels", maxIn);
        napi_set_named_property(env, obj, "maxOutputChannels", maxOut);
        napi_set_element(env, arr, (uint32_t)i, obj);
    }
    return arr;
}

static napi_value NapiListAudioApps(napi_env env, napi_callback_info) {
    auto apps = listAudioApps();
    napi_value arr;
    napi_create_array_with_length(env, apps.size(), &arr);
    for (size_t i = 0; i < apps.size(); i++) {
        napi_value obj, pid, name, exe;
        napi_create_object(env, &obj);
        napi_create_uint32(env, apps[i].pid, &pid);
        napi_create_string_utf8(env, apps[i].name.c_str(),
                                apps[i].name.size(), &name);
        napi_create_string_utf8(env, apps[i].exe.c_str(),
                                apps[i].exe.size(), &exe);
        napi_set_named_property(env, obj, "pid", pid);
        napi_set_named_property(env, obj, "name", name);
        napi_set_named_property(env, obj, "exe", exe);
        napi_set_element(env, arr, (uint32_t)i, obj);
    }
    return arr;
}
...
static napi_value NapiCreateTunnel(napi_env env, napi_callback_info info) {
    size_t argc = 6;
    napi_value args[6];
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);

    /* args: tunnelId, inputs[], outputDeviceId, framesPerBuffer,
             channelCount, duckingConfig */
    std::string tunnelId = NapiGetString(env, args[0]);

    /* Parse inputs array */
    std::vector<InputConfig> inputs;
    uint32_t inputLen = 0;
    napi_get_array_length(env, args[1], &inputLen);
    for (uint32_t i = 0; i < inputLen; i++) {
        napi_value elem;
        napi_get_element(env, args[1], i, &elem);

        napi_value vDevId, vAppPid, vGain, vPriority;
        napi_get_named_property(env, elem, "deviceId", &vDevId);
        napi_get_named_property(env, elem, "appPid", &vAppPid);
        napi_get_named_property(env, elem, "gain", &vGain);
        napi_get_named_property(env, elem, "priority", &vPriority);

        InputConfig cfg;
        napi_get_value_int32(env, vDevId, &cfg.deviceId);

        /* appPid may be undefined */
        napi_valuetype appPidType;
        napi_typeof(env, vAppPid, &appPidType);
        if (appPidType == napi_number) {
            napi_get_value_int32(env, vAppPid, &cfg.appPid);
        } else {
            cfg.appPid = 0;
        }

        double gain;
        napi_get_value_double(env, vGain, &gain);
        cfg.gain = (float)gain;

        napi_get_value_bool(env, vPriority, &cfg.priority);

        inputs.push_back(cfg);
    }

    int32_t outputDeviceId;
    napi_get_value_int32(env, args[2], &outputDeviceId);

    int32_t framesPerBuffer = 0;
    napi_valuetype fpbType;
    napi_typeof(env, args[3], &fpbType);
    if (fpbType == napi_number) {
        napi_get_value_int32(env, args[3], &framesPerBuffer);
    }

    int32_t channelCount = 0;
    napi_valuetype chType;
    napi_typeof(env, args[4], &chType);
    if (chType == napi_number) {
        napi_get_value_int32(env, args[4], &channelCount);
    }

    /* Parse ducking config */
    DuckingConfig ducking = {false, 0.15f, 1000.0f};
    napi_valuetype duckType;
    napi_typeof(env, args[5], &duckType);
    if (duckType == napi_object) {
        napi_value vEnabled, vAmount, vRelease;
        napi_get_named_property(env, args[5], "enabled", &vEnabled);
        napi_get_named_property(env, args[5], "amount", &vAmount);
        napi_get_named_property(env, args[5], "release", &vRelease);
        napi_get_value_bool(env, vEnabled, &ducking.enabled);
        double amt, rel;
        napi_get_value_double(env, vAmount, &amt);
        napi_get_value_double(env, vRelease, &rel);
        ducking.amount = (float)amt;
        ducking.release = (float)rel;
    }

    Engine::createTunnel(tunnelId, inputs, outputDeviceId,
                          framesPerBuffer, channelCount, ducking);
    return nullptr;
}

static napi_value NapiDestroyTunnel(napi_env env, napi_callback_info info) {
    size_t argc = 1;
    napi_value args[1];
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
    Engine::destroyTunnel(NapiGetString(env, args[0]));
    return nullptr;
}

static napi_value NapiDestroyAllTunnels(napi_env env, napi_callback_info) {
    Engine::destroyAllTunnels();
    return nullptr;
}

static napi_value NapiReloadAllTunnels(napi_env env, napi_callback_info info) {
    size_t argc = 1;
    napi_value args[1];
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
    int32_t fpb = 0;
    napi_get_value_int32(env, args[0], &fpb);
    Engine::reloadAllTunnels(fpb);
    return nullptr;
}

static napi_value NapiSetTunnelMuted(napi_env env, napi_callback_info info) {
    size_t argc = 2;
    napi_value args[2];
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
    bool muted;
    napi_get_value_bool(env, args[1], &muted);
    Engine::setTunnelMuted(NapiGetString(env, args[0]), muted);
    return nullptr;
}

static napi_value NapiSetTunnelGain(napi_env env, napi_callback_info info) {
    size_t argc = 2;
    napi_value args[2];
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
    double gain;
    napi_get_value_double(env, args[1], &gain);
    Engine::setTunnelGain(NapiGetString(env, args[0]), (float)gain);
    return nullptr;
}

static napi_value NapiSetTunnelInputGain(napi_env env, napi_callback_info info) {
    size_t argc = 3;
    napi_value args[3];
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
    int32_t idx;
    ...
    napi_get_value_double(env, args[2], &gain);
    Engine::setTunnelInputGain(NapiGetString(env, args[0]), idx, (float)gain);
    return nullptr;
}

static napi_value NapiSetTunnelInputPriority(napi_env env,
    ...
    bool priority;
    napi_get_value_int32(env, args[1], &idx);
    napi_get_value_bool(env, args[2], &priority);
    Engine::setTunnelInputPriority(NapiGetString(env, args[0]), idx, priority);
    return nullptr;
}

static napi_value NapiSetTunnelDucking(napi_env env, napi_callback_info info) {
    size_t argc = 2;
    napi_value args[2];
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);

    DuckingConfig cfg = {false, 0.15f, 1000.0f};
    napi_value vEnabled, vAmount, vRelease;
    napi_get_named_property(env, args[1], "enabled", &vEnabled);
    napi_get_named_property(env, args[1], "amount", &vAmount);
    napi_get_named_property(env, args[1], "release", &vRelease);
    napi_get_value_bool(env, vEnabled, &cfg.enabled);
    double amt, rel;
    napi_get_value_double(env, vAmount, &amt);
    napi_get_value_double(env, vRelease, &rel);
    cfg.amount = (float)amt;
    cfg.release = (float)rel;

    Engine::setTunnelDucking(NapiGetString(env, args[0]), cfg);
    return nullptr;
}

static napi_value NapiGetTunnelSampleRate(napi_env env,
                                           napi_callback_info info) {
    size_t argc = 1;
    napi_value args[1];
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
    int sr = Engine::getTunnelSampleRate(NapiGetString(env, args[0]));
    napi_value result;
    if (sr > 0) {
        napi_create_int32(env, sr, &result);
    } else {
...
static napi_value NapiGetTunnelChannelCount(napi_env env,
                                             napi_callback_info info) {
    size_t argc = 1;
    napi_value args[1];
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
    int ch = Engine::getTunnelChannelCount(NapiGetString(env, args[0]));
    napi_value result;
    if (ch > 0) {
        napi_create_int32(env, ch, &result);
    } else {
        napi_get_null(env, &result);
    }
    return result;
...
static napi_value NapiRegisterLevelCallback(napi_env env,
                                             napi_callback_info info) {
    size_t argc = 1;
    napi_value args[1];
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);

    /* Release old TSFN if any */
    if (g_levelTsfn) {
        napi_release_threadsafe_function(g_levelTsfn, napi_tsfn_release);
        g_levelTsfn = nullptr;
    }

    napi_value name;
    napi_create_string_utf8(env, "levelCallback", NAPI_AUTO_LENGTH, &name);
    napi_create_threadsafe_function(
        env, args[0], nullptr, name,
        0, 1,   /* unlimited queue, 1 initial thread ref */
        nullptr, nullptr, nullptr,
        LevelTsfnCallback, &g_levelTsfn
    );

    Engine::setLevelCallback([](const std::string& tunnelId, float level) {
        if (!g_levelTsfn) return;
        auto* ld = new LevelData{tunnelId, level};
        napi_call_threadsafe_function(g_levelTsfn, ld, napi_tsfn_nonblocking);
    });
...
static napi_value ModuleInit(napi_env env, napi_value exports) {
    ...
    EXPORT_FN("getAudioDevices",        NapiGetAudioDevices);
    EXPORT_FN("listAudioApps",          NapiListAudioApps);
    EXPORT_FN("createTunnel",           NapiCreateTunnel);
    EXPORT_FN("destroyTunnel",          NapiDestroyTunnel);
    EXPORT_FN("destroyAllTunnels",      NapiDestroyAllTunnels);
    EXPORT_FN("reloadAllTunnels",       NapiReloadAllTunnels);
    EXPORT_FN("setTunnelMuted",         NapiSetTunnelMuted);
    EXPORT_FN("setTunnelGain",          NapiSetTunnelGain);
    EXPORT_FN("setTunnelInputGain",     NapiSetTunnelInputGain);
    EXPORT_FN("setTunnelInputPriority", NapiSetTunnelInputPriority);
    EXPORT_FN("setTunnelDucking",       NapiSetTunnelDucking);
    EXPORT_FN("getTunnelSampleRate",    NapiGetTunnelSampleRate);
    EXPORT_FN("getTunnelChannelCount",  NapiGetTunnelChannelCount);
    EXPORT_FN("registerLevelCallback",  NapiRegisterLevelCallback);
