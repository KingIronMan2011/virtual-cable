...
pub fn build_tunnel(
    ...
    for input_cfg in inputs {
        ...
        if input_cfg.app_pid > 0 {
            ...
            capture.start(callback, channels as i32, sample_rate);
            ...
            crate::audio::device_enum::get_cpal_device_by_id(input_cfg.device_id)
        {
            ...
            let in_rate = in_cfg.sample_rate as f32;
            let out_rate = sample_rate as f32;
    ...
    let mut scratch_buffers: Vec<Vec<f32>> =
        (0..mixer_inputs.len()).map(|_| vec![0.0; 4096]).collect();
            ...
            move |data: &mut [f32], _| {
                ...
                for (idx, inp) in mixer_inputs.iter_mut().enumerate() {
                    ...
                    if duck_enabled && inp.priority.load(Ordering::Relaxed) {
                        let sum_sq: f32 = scratch_buffers[idx][..data.len()]
                            .iter()
                            .map(|&s| s * s)
                            .sum();
                        let rms = (sum_sq / data.len() as f32).sqrt();
                        if rms > 0.02 {
                            // Threshold for ducking trigger
